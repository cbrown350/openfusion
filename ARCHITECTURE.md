...
`isConfigured()` = `candidates.length ≥ 2 && judge set && every referenced provider that needs a key has one` (keyless providers — e.g. the local `rapid-mlx` server — are exempt, see `KEYLESS_PROVIDERS` in `src/providers/custom-providers.ts`; keyed providers like `ollama-cloud` are not). Minimum **2**, maximum **5** candidates (enforced in schema + UI).
...
| GET | `/api/providers` | all providers (pi-ai built-ins + custom) with metadata `{providers: {id, name, discoverable}[]}` |
...
| GET | `/api/providers/:p/discover` | explicit retry for local discoverable providers (`404` for non-discoverable; `502 {error}` on fetch failure) |
