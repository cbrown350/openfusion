...
### Added (Playground — run fusions from the browser)
- **Playground tab** — a new first/default dashboard tab (`/` now renders the Playground, ahead of Dashboard, Settings, Errors). A Google-AI-Studio-style MCP-client UI letting a user run a fusion directly from the browser — type a prompt (+ optional context + optional persona override), hit Run, watch live fan-out → analysis → synthesis progress, then read the synthesized answer plus the per-candidate and judge breakdown inline. Zero MCP client setup. The run is logged identically to an MCP fusion and shows up in the Dashboard/Errors activity views like any other.
...
### Changed (Playground redesign)
- **Playground redesigned to a Studio workbench** (`ui/src/pages/Playground.tsx`) — a composer-on-top + two-column-below layout (left rail: example prompts + reloadable session history; right: live progress → run-metadata bar → synthesized answer → collapsible candidate/judge breakdown). The grid collapses to one column below 1040px. Live progress reuses the real engine state via the shared `FusionProgress`/`PhaseBar` (extracted from the Dashboard) instead of a feigned indicator. The Run button drives the real `POST /api/fusion` → poll `GET /api/runtime` → `GET /api/activity/:id`; because POST returns the `activityId` only at completion, the Playground snapshots the known in-flight fusion ids just before kickoff and identifies "mine" during polling, so progress is never stuck on "Starting…" for a whole run. Design record: `specs/0091-playground-ui-revamp/playground-handoff.md` + `openfusion-playground.html`.
- **Generations tab removed** — folded into the Playground, whose history rail + inline breakdown (the same `FusionBreakdown` components) cover reading any past fusion's candidate/judge output. `/generations` redirects to `/playground` so bookmarks don't break. Nav shrinks to four tabs (Playground · Dashboard · Settings · Errors). The `persona_source` chip-suffix logic that lived in `Generations.tsx` now lives in `Errors.tsx` (covered by `tests/fusion-persona-source.test.ts`).
- **Chat-style composer + file attachments** — a single chat-style input box (paperclip attaches local text/PDF files; `ui/src/lib/fileToText.ts` parses them client-side via `pdfjs-dist` with a dynamic import so the ~300KB parser is code-split out of the main bundle) whose extracted text is concatenated into the fusion `context`. Attachments are ephemeral (never persisted — like all context, only a `has_context` flag is stored). A soft amber token-cost warning fires past ~100k estimated tokens. `ui/src/components/ComposerAttachments.tsx` owns the chip row + hidden file input.
- **Shared live-progress component** — `PhaseBar` (segmented phase bar + candidate dot-row) is extracted from `Dashboard.tsx` into `ui/src/components/FusionProgress.tsx`, now the single source of truth used by both the Dashboard's server-status widget and the Playground's result pane.
- **`.btn-primary` made self-contained** (`ui/src/index.css`) — the Run button (and the amber banner `<Link>`s) previously rendered as a gradient nub because `.btn-primary` only carried the gradient, leaning on a paired `.btn` for the box. Split: gradient/color stays universal on `.btn-primary`; the self-contained box (padding/radius/states/`.is-running` spinner) is scoped to `.btn-primary:not(.btn)`, so the four pages that pair `.btn .btn-primary` (Candidates/Judge/Personas/API Keys) are byte-identical.
- **Route-specific shell width** — `:root { --shell-max: 64rem }` (was `max-w-5xl` = 64rem; every non-Playground page stays byte-identical); the Playground widens it to 80rem via a layout effect on mount and resets on unmount. `App.tsx` gains only the four `max-w-5xl → max-w-[var(--shell-max)]` swaps — no nav, logo, pills, or routing changes beyond the Generations removal above.
- **Markdown rendering migrated** (`ui/src/components/GenerationText.tsx`) — the hand-rolled renderer is replaced by `react-markdown` + `remark-gfm` (full GFM, including tables/strikethrough/autolinks), rendering to React elements without `dangerouslySetInnerHTML`. Component overrides match the prior hand-rolled styling. Net ~125 lines smaller.
...
- **Raised API body limit** — `express.json` limit raised `1mb → 25mb` (`src/server/ui-server.ts`) so file-attachment contexts aren't rejected as 413. The UI enforces sane usage via a token-cost warning; the cap only guards against OOM. Covered by a regression test (`tests/playground-api.test.ts (f)`) asserting a ~2MB context body is accepted and flows through to `has_context`.

### Added (Playground redesign dependencies)
- **`pdfjs-dist`**, **`react-markdown`**, **`remark-gfm`** added as UI dependencies (`ui/package.json`).
...
[0.3.1]: https://github.com/hashangit/openfusion/releases/tag/v0.3.1
